...
test_that("print.staleness_report() shows 'All entries are current' when none stale", {
  # Build a mock clean staleness report
  mock <- data.frame(
    key = "dplyr::filter",
    pkg = "dplyr",
    fn = "filter",
...
test_that("print.staleness_report() shows stale ceiling details", {
  mock <- data.frame(
    key = "dplyr::filter",
    pkg = "dplyr",
    fn = "filter",
...
test_that("print.staleness_report() shows stale floor details", {
  mock <- data.frame(
    key = "dplyr::filter",
    pkg = "dplyr",
    fn = "filter",
...
test_that("check_db_staleness() returns empty df when all entries closed", {
  # All base R entries are closed -- use only those
  result <- check_db_staleness(
    packages = "stats",
    source   = "installed",
    verbose  = FALSE
  )
  # Closed entries are skipped -- result should be empty
...
test_that(".assess_staleness() returns 'ok' when current <= to_version", {
  ...
  expect_equal(reproducr:::.assess_staleness("3.6.0", "3.6.9"), "ok")
...
test_that(".assess_floor_staleness() returns 'stale' when gap >= threshold", {
  expect_equal(
    reproducr:::.assess_floor_staleness("4.0.0", "2.0.0", 2L), "stale"
  )
  expect_equal(
    reproducr:::.assess_floor_staleness("4.7.2", "3.0.2", 1L), "stale"
  )
})

test_that(".assess_floor_staleness() returns 'ok' when gap < threshold", {
  expect_equal(
    reproducr:::.assess_floor_staleness("4.7.2", "3.0.2", 2L), "ok"
  )
  expect_equal(
    reproducr:::.assess_floor_staleness("1.2.0", "1.0.0", 1L), "ok"
  )
})

test_that(".assess_floor_staleness() returns 'ok' when threshold is Inf", {
  expect_equal(
    reproducr:::.assess_floor_staleness("9.0.0", "1.0.0", Inf), "ok"
  )
})

test_that(".assess_floor_staleness() returns 'ok' for NA current version", {
  expect_equal(
    reproducr:::.assess_floor_staleness(NA_character_, "1.0.0", 1L), "ok"
  )
})
